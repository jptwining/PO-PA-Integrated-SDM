setwd('./data')
PAdata <- read.csv("allNY_2013-2021_bobcatPArecords_pixelID_10kmgrid.csv")
POdata <- read.csv("allNY_2013_2021_bobcatPOrecord_pixelID_10kmgrid_noweeklydups.csv")
...
#Remove 2014 
remove2014 <- which(PAdata$year == 2014)
PAdata <- PAdata[-remove2014,]
...
for(i in 1:nrow(PAdata)){ #loop through each row
  ...
  y.new[this.site,1:3,this.year]=as.numeric(PAdata[i,7:9]) #force numeric
}


for(t in 1:n.year){
  print(c(sum(y.new[1:J[t],,t], na.rm=TRUE),sum(PAdata[PAdata$year==years[t],7:9], na.rm=TRUE)))
}
...
out = for (chain in 1:n.chains){
  source("./models/nimble_ppp_integrated_model_bobcat_10km_indexingoveryear_randomeffects_centered.R")
  ...
  constants=list(npixel = G, conifmixed = conifmixed.new, deciduous = deciduous.new, edge = edge.new, POden = POcount.new, pa_pixel = PAdata$FID_allNY_10km,
                 ...
                 CONSTANT = constant, road = road.new, date = date.new, 
                 nsurveys = dim(y.new)[2], occ = occ.new, J = J, nyears = n.year, cell_area = log(10)) #constants go here
  #inits
  Niminits=list(alpha=c(0,0,0), alpha0 = 0, beta=c(0,0,0,0), beta0 = 0, gamma0 = 0, gamma = 0, beta0.it = beta0.it, sd.lam = 1, gamma0.it = gamma0.it, sd.b = 1, alpha0.it = alpha0.it, sd.p = 1) #setting initial vales. When working on log or logit scale, 0 is usually a half decent starting value.
  ...
  #block these due to strong posterior correlation. These are largest offenders, can block more.
  conf$removeSampler(c("alpha0","beta0"))
  conf$addSampler(target = c("alpha0","beta0"),
                  type = 'RW_block',control = list(adaptive=TRUE),silent = TRUE)
  
  # #block these due to strong posterior correlation. These are largest offenders, can block more.
  # conf$removeSampler(c("gamma0", "gamma0.it"))
  # conf$addSampler(target = c("gamma0", "gamma0.it"),
  #                 type = 'RW_block',control = list(adaptive=TRUE),silent = TRUE)
  # 
...
n.iter <- 100000 #number of iterations in thinned chains
n.burn <- 50000 #per chain burn in

#make a list including all three chains
...
#save the chains and whatnot for processing
save(allthestuff, "bobcat_SDM_chainsandstatelist_randomeffectsmonitored.RData")
