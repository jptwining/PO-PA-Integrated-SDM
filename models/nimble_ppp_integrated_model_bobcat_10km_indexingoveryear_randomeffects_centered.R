...
NimModel <- nimbleCode({
 ...
 #Priors and hyperpriors for PO observation model
  gamma0 ~ dnorm(0, 0.1)
  gamma[1] ~ dnorm(0, 0.1)
  tau.b <- pow(sd.b, -2)
  sd.b ~ dunif(0, 2)
  ...
  for(pixel in 1:npixel){
    for (t in 1:nyears){    
      ...
      log(lambda[pixel,t]) <- beta0.it[t] + beta[1] * conifmixed[pixel] + beta[2] * deciduous[pixel] + beta[3] * edge[pixel]  + beta[4] * road[pixel] + cell_area
