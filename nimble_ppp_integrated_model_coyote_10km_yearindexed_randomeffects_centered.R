...
NimModel <- nimbleCode({
  ...
  # Nimble version of the Fidino (2021) model.
  #
  # --- Priors ---
  #Occupancy model
...
#   for (j in 1:J){
#     for (k in 1:nsurveys) {
#       Ysim[j,k] ~ dbern((p[j,k] * K2D[j,k]) * z[pa_pixel[j]])
#       Yexp[j,k] <- (p[j,k] * K2D[j,k]) * z[pa_pixel[j]]
#       
#       #components for test 1
#       err1obs[j,k] <- (sqrt(y[j,k]) - sqrt(Yexp[j,k]))^2 
#       err1sim[j,k] <- (sqrt(Ysim[j,k]) - sqrt(Yexp[j,k]))^2
#     }
#   }
#   #fit diagnostic totals
#   T1obs <- sum(err1obs[1:J, 1:nsurveys])
#   T1sim <- sum(err1sim[1:J, 1:nsurveys])
# }
)
